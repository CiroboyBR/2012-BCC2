...
BITMAP *RetornaPersonagem( float *nIndex, int nDirecao )
{
    ...
    if( nDirecao == 2)
    {
        if( *nIndex <= 0 || *nIndex >= 8 ) {
            pPersonagem = load_bitmap( "Up1.bmp", NULL );
            *nIndex = 1;
        }
    ...
    {
        if( *nIndex >= 0 || *nIndex <= -8  ) {
            pPersonagem = load_bitmap( "Dowm0.bmp", NULL );
            *nIndex = -1;
        }
    ...
    {
        if( *nIndex <= 0 || *nIndex >= 8 ) {
            pPersonagem = load_bitmap( "Right1.bmp", NULL );
            *nIndex = 1;
        }
    ...
    {
        if( *nIndex >= 0 || *nIndex <= -8  ) {
            pPersonagem = load_bitmap( "Left1.bmp", NULL );
            *nIndex = -1;
        }
...
int ExecuteCorredorBolhas( )
{
    ...
    // Pontuação
    textprintf_ex(bmp,oFont,800,0,makecol(0,0,0),-1, "Gramado: %i", nVal_TelaEsq );
    textprintf_ex(bmp,oFont,800,40,makecol(0,0,0),-1,"Casa:   %i", nVal_TelaCim );
    textprintf_ex(bmp,oFont,800,80,makecol(0,0,0),-1,"Deserto: %i", nVal_TelaDir);
...
int ExibirBolha( int nQtBolha, FONT *oFont, BITMAP *bmp )
{
    ...
    for( i = 0; i < nQtBolha ; i ++ )
    {
        if( VetorBolha[i].nVal == -1 )
        {
            ...
            textprintf_ex( bmp, oFont,VetorBolha[i].nX, VetorBolha[i].nY, makecol(120,110,200),-1, "%i", VetorBolha[i].nVal );
        }
    }
